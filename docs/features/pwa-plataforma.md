...
## Flujos principales
1. **Request autenticado**: middleware → excluye `/login`, `/auth/*`, `/api/*`, `/_next/*` (`debeSaltearElGate`, prefijos anclados) y los archivos con extensión (el `matcher`, anclado al final del path) → `supabase.auth.getUser()` → sin usuario redirige a `/login` → con usuario y fuera de `/onboarding`, consulta `users.onboarding_completed`; si no completó, redirige a `/onboarding` **copiando las cookies** de sesión al redirect (evita loops de sesión).
...
4. **PWA**: en producción el SW cachea navegación front-end agresivamente y recarga al volver online; en dev no hay SW (evita cache fantasma). El manifest se sirve desde `src/app/manifest.ts`, que es la única fuente (el `public/site.webmanifest` legacy se borró el 2026-08-22 junto con los íconos viejos).
   ⚠️ **Al cerrar sesión hay que vaciar ese caché**: `limpiarCachesDeLaApp()` (`lib/pwa/caches.ts`) corre en las **dos** salidas — «Cerrar sesión» y «Borrar mi cuenta»— del lado del **cliente y antes** de llamar a la server action, porque `caches` es del navegador y las dos actions redirigen. Sin eso, en un dispositivo compartido el próximo en usarlo puede ver por un instante una pantalla del anterior servida de caché (los datos no vuelven —la sesión se fue—, pero el HTML sí). Auditoría 2026-08-26, L6. La función nunca lanza: si el navegador no tiene CacheStorage o un borrado falla, la salida de sesión sigue igual.
