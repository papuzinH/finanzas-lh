...
3. **Mensualidades automáticas de crédito** (`syncAutomaticRecurringCharges`): una mensualidad facturada en tarjeta no se paga, se debita cuando cierra el resumen. Las de crédito nacen solas como transacción fechada al vencimiento del resumen que le toca al cobro, con su `cycle_id` (`expectedChargeDatePorCiclo`; el día de cobro es `billing_day ?? 1` clampeado al último día del mes, y `expectedChargeDate` sobre los defaults de la tarjeta queda como fallback si no hay ciclo), igual que cuotas y compras variables. Se dispara desde `fetchAllData()` **una vez por carga de la app** (flag de módulo `automaticChargesSynced`), nunca en cada refetch. Sólo aplica a planes **mensuales** sobre tarjetas con ciclo cargado; el resto sigue con el toggle manual. Piso: el mes más tardío entre la creación del plan y el primer ingreso del usuario. **Cobertura de un mes**: cada transacción del plan aporta DOS claves —su `cycle_id` y el mes de su `date`— y el mes está cubierto si coincide **cualquiera** de las dos con lo que predice `expectedChargeDatePorCiclo`. No se particiona: una fila con resumen aporta igual su clave de mes. Hacen falta las dos porque declarar un resumen puede moverlo de mes calendario (la de `cycle_id` lo salva), porque hay transacciones viejas sin `cycle_id` (la de mes las salva) y porque declarar o realinear puede mover el día de cobro a **otro** resumen que el que tiene la transacción ya posteada (ahí sólo la salva la clave de mes). El mes y no la fecha exacta, a propósito: una fecha editada a mano sigue contando. Límite conocido: si el resumen nuevo Y su mes de vencimiento difieren los dos de los de la transacción posteada, el cargo se duplica igual.
