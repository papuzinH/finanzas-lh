...
| `src/lib/finance/recurring.ts` | `isAutomaticPlan`, `expectedChargeDate`, `computeMissingAutomaticCharges` (mensualidades de crédito que se postean solas) |
...
3. **Mensualidades automáticas de crédito** (`syncAutomaticRecurringCharges`): una mensualidad facturada en tarjeta no se paga, se debita cuando cierra el resumen. Las de crédito nacen solas como transacción fechada al vencimiento (`expectedChargeDate` = `billing_day ?? 1` clampeado al último día del mes, pasado por `calculateCreditPaymentDate`), igual que cuotas y compras variables. Se dispara desde `fetchAllData()` **una vez por carga de la app** (flag de módulo `automaticChargesSynced`), nunca en cada refetch. Sólo aplica a planes **mensuales** sobre tarjetas con ciclo cargado; el resto sigue con el toggle manual. Piso: el mes más tardío entre la creación del plan y el primer ingreso del usuario. Cobertura de un mes = existe transacción del plan en el mismo mes del vencimiento esperado (por mes, no por fecha exacta: una fecha editada a mano sigue contando).
