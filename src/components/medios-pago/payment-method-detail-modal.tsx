      ...
      <DialogContent className="max-h-[90vh] overflow-hidden flex flex-col gap-0 p-0 sm:max-w-[500px] bg-surface border-border text-text">
        <DialogHeader className="p-6 pb-0 flex-shrink-0">
          <div className="flex items-center gap-3 mb-2">
            <div className={cn(
              ...
              isCredit ? "bg-accent/10 text-accent-deep" : "bg-accent/10 text-accent-deep"
            )}>
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-surface-2/50 p-4 rounded-2xl border border-border/50">
              <p className="text-[10px] text-muted uppercase mb-1 font-semibold">
                {isCredit ? 'Consumo Total' : 'Saldo Actual'}
              </p>
              <p className={cn(
                "text-xl font-bold font-mono",
                status.projectedTotal < 0 ? "text-bad" : "text-good"
              )}>
            ...
            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              {history.length > 0 ? (
                history.map((t) => {
                  ...
                  return (
                    <div key={t.id} className="flex items-center justify-between p-3 rounded-xl bg-surface-2/30 border border-border/30 hover:bg-surface-2/30 transition-colors group">
                      <div className="flex items-center gap-3">
                        <div className={cn(
                          "p-2 rounded-lg",
                          t.type === 'income' ? "bg-good/10 text-good" : "bg-surface-2 text-muted"
                        )}>
                ...
                <div className="text-center py-8 bg-surface-2/20 rounded-2xl border border-dashed border-border">
            ...
            <div className="space-y-3 pt-4 border-t border-border/50">
              ...
              <div className="grid grid-cols-1 gap-2">
                {subscriptions.map((sub) => (
                  <div key={sub.id} className="flex items-center justify-between p-2 px-3 rounded-lg bg-accent/5 border border-accent/10">
                    <span className="text-xs text-text">{sub.description}</span>
                    <span className="text-xs font-mono font-bold text-accent-deep">{formatCurrency(sub.amount)}</span>
                  </div>
                ))}
              </div>
            </div>
