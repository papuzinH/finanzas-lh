...
- **GOTCHA `user_id`** (fuente de 5 bugs silenciosos ya corregidos): `transactions`/`payment_methods`/`recurring_plans`/`installment_plans` filtran por el **id interno de `users`** (`users.id` → `ctx.userId`); `categories`/`internal_transfers`/`savings_goals`/`category_budgets`/`investment_assets`/`investment_transactions` por el **UUID de auth** (`getAuthUserId()` en handlers, `ctx.authUserId` en tools). Confundirlos produce queries que nunca matchean, sin error. OJO (verificado contra la DB real 2026-07-08): `users.id` es un **UUID que ES el `auth.uid()`** (FK directa a `auth.users(id)`), y TODOS los ids de las tablas de la app son UUID — `types/database.ts` está **regenerado desde el schema real** (los ids numéricos solo existen en las tablas `legacy_*`). En runtime `ctx.userId` y `ctx.authUserId` valen lo mismo, pero mantené la convención por tabla igual: las FKs difieren. NUNCA filtres por `users.auth_user_id` (columna vestigial).
