...
- **Un resumen vencido sin pago NO desaparece** (2026-09-01): `getCreditCycleDates` avanza al siguiente resumen al día siguiente del vencimiento, así que el viejo se esfumaba — el compromiso se liberaba solo, la plata nunca salía de ninguna cuenta y **el disponible subía por el monto entero del resumen, todos los meses y en silencio** (E11). Ahora `computePendingCreditCards` devuelve también el ciclo inmediato anterior si quedó impago, marcado con **`isOverdue`**, y sigue contando como compromiso. ⚠️ **El piso es tan importante como el mecanismo**: sólo se retiene lo que venció DESPUÉS del último `initial_balance_at` entre las cuentas del bolsillo — un resumen anterior al ancla ya está adentro de ese saldo declarado y retenerlo lo restaría dos veces (el agujero de −$850.613 del 21-ago). Sin ninguna cuenta anclada no se retiene nada. **Limitación asumida**: sólo un ciclo hacia atrás; con dos meses seguidos sin marcar, el más viejo se pierde. La contracara visible es `OverdueCardPaymentBanner` en el home, que **no se puede descartar** a propósito: es la explicación de por qué el disponible está más bajo y la única vía para saldarlo. Fijado en E12.
