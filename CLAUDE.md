...
- **Declarar el resumen** (Plan 2, `declararCiclo` → `lib/ciclos/declarar.ts` → `guardarDeclaracion`): el usuario carga a mano el cierre/vencimiento que le imprime el banco (Ley 25.065 art. 23). Corrige el resumen del **mismo mes calendario** si ya existe (nunca inserta uno paralelo — la unique es `(payment_method_id, closing_date)`) y lo marca `source: 'declared'`; **no reasigna ninguna transacción**. Tres lugares: la ficha de la tarjeta en `/ajustes/medios` (`institutional-card.tsx` → botón «Corregir fechas» → `EditarCicloDialog`, siempre disponible mientras haya un resumen vigente materializado), el paso opcional «Lo tengo a mano, lo cargo» al marcar un pago (`DeclararProximoCiclo`, montado en el `AlertDialog` de Compromisos y en `RegisterCardPaymentDialog`) y el recordatorio que aparece el día que el resumen cierra (`RecordatorioDeclararCiclo` en `/compromisos`, uno por tarjeta — el último resumen `generated` ya cerrado). **Confirmar exige un gesto**: el paso opcional arranca cerrado y sólo dispara `onDeclarar` si el usuario lo abre, así que darle «Pagué»/«Registrar pago» de largo nunca convierte una estimación en dato declarado. «Ahora no» en el recordatorio persiste en `credit_card_cycles.reminder_dismissed_at` (no en localStorage): reaparece en cualquier dispositivo hasta que se declare, pero no en el mismo tras posponerlo.
...
- ⚠️ **Ciclos de tarjeta — `20260902151524_credit_card_cycles_reminder` es BLOQUEANTE: va a producción ANTES de publicar**, no después. `generarCiclos` manda `reminder_dismissed_at: null` en **cada fila** que inserta (`lib/finance/cycles.ts`) y ese payload va al upsert de `asegurarCiclos`. Si el código sale sin la columna, PostgREST rechaza el insert, `asegurarCiclos` lanza y **se cae el alta de cualquier movimiento de crédito**, por pantalla y por chat. No es un degradado silencioso: es el camino principal roto.
